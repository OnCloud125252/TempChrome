...
- [ ] 1.1 Create `raycast/package.json` with these fields: `name: "tempchrome"`, `title: "TempChrome"`, `description: "Launch Chromium with temporary, isolated profiles"`, `author: "oncloud"`, `license: "MIT"`, `categories: ["Applications", "Developer Tools"]`, `icon: "icon.png"`, scripts `{ "build": "ray build -e dist", "dev": "ray develop", "lint": "ray lint", "fix-lint": "ray lint --fix", "publish": "ray publish" }`, dependencies `{ "@raycast/api": "^1.80.0", "@raycast/utils": "^1.17.0" }`, devDependencies `{ "@raycast/eslint-config": "^1.0.11", "@types/node": "^20.8.10", "@types/react": "^18.3.3", "eslint": "^8.57.0", "prettier": "^3.3.3", "typescript": "^5.4.5" }`. Leave the `commands` and `preferences` arrays for tasks 1.2 and 1.3.

- [ ] 1.2 Add a `commands` array to `raycast/package.json` with exactly these two entries:
...
- [ ] 1.3 Add a `preferences` array to `raycast/package.json` with exactly these two entries:
...
- [ ] 1.4 Create `raycast/tsconfig.json` with content `{ "extends": "@raycast/tsconfig/base.json", "compilerOptions": { "jsx": "react-jsx", "strict": true, "target": "ES2020", "lib": ["ES2020", "DOM"], "esModuleInterop": true, "moduleResolution": "node", "skipLibCheck": true }, "include": ["src/**/*"] }`. If `@raycast/tsconfig` is unavailable, inline equivalent compiler options.

- [ ] 1.5 Create `raycast/.eslintrc.json` with content `{ "extends": ["@raycast"] }`.

- [ ] 1.6 Create `raycast/.prettierrc` with content `{ "semi": true, "singleQuote": false, "printWidth": 100, "tabWidth": 2, "trailingComma": "all" }`.

- [ ] 1.7 Create `raycast/.gitignore` containing `node_modules/`, `dist/`, `.DS_Store`, `.raycast/`, each on its own line.

- [ ] 1.8 Create `raycast/assets/` directory. Add a placeholder `icon.png` (512×512). Document at the top of `raycast/README.md` that the final icon should be an indigo (`#4F46E5`) background with a white globe+stopwatch glyph. A solid-color PNG is acceptable until the final art is ready.

- [ ] 1.9 From `raycast/`, run `npm install`. Verify `raycast/node_modules/@raycast/api/package.json` exists and that `npm run lint` exits 0 on the empty `src/` (it may warn about no source files; no errors).

## 2. Shared Modules

- [ ] 2.1 Create `raycast/src/constants.ts` exporting these named constants exactly:
...
- [ ] 2.2 Create `raycast/src/preferences.ts` exporting:
...
- [ ] 2.3 Create `raycast/src/chromium.ts` exporting the following async helpers:
...
- [ ] 2.4 Create `raycast/src/process-check.ts` exporting:
...
- [ ] 2.5 Create `raycast/src/trash.ts` exporting:
...
- [ ] 2.6 Create `raycast/src/auto-cleanup.ts` exporting:
...
- [ ] 2.7 Create `raycast/src/profiles.ts` exporting:
...
- [ ] 3.1 Create `raycast/src/launch.ts` exporting:
...
- [ ] 3.2 Manual verification: run `cd raycast && npm run dev`. In Raycast, trigger "Launch TempChrome". Verify: (a) HUD appears, (b) a new directory appears under `/tmp/tempchrome_profile/` with a 10-character lowercase-alphanumeric name, (c) Chromium opens with a fresh profile.

## 4. TempChrome Root Command (view, List)

- [ ] 4.1 Create `raycast/src/tempchrome.tsx` default-exporting a React functional component `Command()` that returns `<List>` containing four `<List.Item>` elements in this order:
...
- [ ] 4.2 Define `handleLaunch` in `tempchrome.tsx` — imports and calls `quickLaunch()` from `./launch`. After `await quickLaunch()`, close the Raycast window via `popToRoot({ clearSearchBar: true })` so the user lands back on root.

- [ ] 4.3 Define `handleInstall` in `tempchrome.tsx` per task 7.1 (implementation lives in this file; referenced from task section 7 for clarity).
...
- [ ] 5.2 Implement `handleSubmit(values: FormValues)` inside `LaunchOptionsForm.tsx`:
...
- [ ] 6.1 Create `raycast/src/ProfileList.tsx` default-exporting a React component that:
...
- [ ] 6.4 Implement `handleDelete(profile: ProfileInfo)`:
...
- [ ] 6.5 Implement `handleDeleteAll(profiles: ProfileInfo[])`:
...
- [ ] 6.6 Implement `handleCleanupStale`:
...
- [ ] 7.1 In `raycast/src/tempchrome.tsx`, implement `async function handleInstall(): Promise<void>`:
...
- [ ] 8.1 From `raycast/`, run `npm run lint`. Fix all errors until the command exits 0. Warnings may be addressed later.

- [ ] 8.2 From `raycast/`, run `npm run build`. Verify the command exits 0 and produces a `raycast/dist/` directory containing the compiled commands.
