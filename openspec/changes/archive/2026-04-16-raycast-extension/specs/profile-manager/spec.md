...
The system SHALL, on mount and on every refresh, enumerate subdirectories of `tempBaseDir` and render one `List.Item` per subdirectory. Items SHALL be sorted by `fs.stat.birthtime` descending (newest first).
